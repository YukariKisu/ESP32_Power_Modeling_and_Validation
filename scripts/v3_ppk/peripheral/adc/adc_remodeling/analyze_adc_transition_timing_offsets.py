...
OUTPUT_DIR = Path(
    ...
    "transition_timing_offset_diagnostics"
)
...
def find_local_50_crossing(
    ...
    """
    Detect measured 50% crossing around one nominal transition.
    """
    dt_s = float(np.median(np.diff(time_s)))
...
def collect_transition_offsets(
    time_s: np.ndarray,
    ...
    """
    Calculate measured 50% crossing minus nominal firmware edge
    for every burst/event.
    """
...
def summarize_transition_offsets(
    ...
    for (
        ...
        if detected > 0:
            offsets = detected_group[
                "offset_ms"
            ].to_numpy(dtype=float)

            offset_mean_ms = float(np.mean(offsets))
            offset_std_ms = (
                float(np.std(offsets, ddof=1))
                if detected > 1
                else 0.0
            )
            offset_min_ms = float(np.min(offsets))
            offset_max_ms = float(np.max(offsets))
            ...
            if detected > 1:
                ...
                fitted = slope * event_index + intercept
                ss_res = float(
                    np.sum(
                        (offsets - fitted) ** 2
                    )
                ...
                ss_tot = float(
                    np.sum(
                        (offsets - np.mean(offsets)) ** 2
                    )
                )

                drift_ms_per_event = float(slope)
                drift_r2 = (
                    1.0 - ss_res / ss_tot
                    if ss_tot > 0
                    else 0.0
            ...
            offset_mean_ms = np.nan
            offset_std_ms = np.nan
            offset_min_ms = np.nan
            offset_max_ms = np.nan
            drift_ms_per_event = np.nan
            drift_r2 = np.nan
        ...
        rows.append(
            {
                ...
                "drift_ms_per_event": drift_ms_per_event,
                "drift_r2": drift_r2,
            }
        )
...
def plot_transition_offsets(
    offset_dataframe: pd.DataFrame,
    condition: str,
    run_id: str,
    output_dir: Path,
) -> None:
    """
    Plot offset versus burst/event index.
    Dashed lines show the current ±2.5 ms search boundary.
    """
    ...
    for transition_type in (
        ...
        subset = offset_dataframe[
            offset_dataframe["transition_type"]
            == transition_type
        ].copy()
        ...
        axis.set_title(
            f"{condition} | {run_id} | "
            f"{transition_type} timing offset"
        )
        axis.set_xlabel(
            "Burst / event index"
        )
        axis.set_ylabel(
            "Measured 50% - nominal edge [ms]"
        )
...
def main() -> None:
    ...
    for condition, input_dir in INPUT_DIRS.items():
        for path in collect_csv_files(
            ...
            try:
                ...
                rise_offsets = collect_transition_offsets(
                    time_s=time_s,
                ...
                fall_offsets = collect_transition_offsets(
                    time_s=time_s,
                    measured_ma=measured_delta_ma,
                ...
                plot_transition_offsets(
                    offset_dataframe=run_offsets,
                    condition=condition,
                    run_id=run_id,
    ...
    summary_dataframe = summarize_transition_offsets(
        offsets_dataframe
    )

    offsets_path = (
        OUTPUT_DIR
        / "transition_timing_offsets.csv"
    )

    summary_path = (
        OUTPUT_DIR
        / "transition_timing_offset_summary.csv"
    )
