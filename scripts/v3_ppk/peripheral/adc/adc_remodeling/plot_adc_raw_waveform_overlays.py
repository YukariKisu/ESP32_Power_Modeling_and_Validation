...
def robust_ylim(runs: list[dict], start_s: float, end_s: float):
    values = []
    ...
    lo = float(np.percentile(merged, 0.2))
    hi = float(np.percentile(merged, 99.8))
    pad = max(0.5, 0.08 * max(hi - lo, 1e-9))
...
def plot_overlay(
    ...
    filename_suffix: str,
    title_suffix: str,
    nominal_edge_s: float | None = None,
):
    ...
    for run in runs:
        run_id = run["run_id"]
        status = RUN_STATUS.get(condition, {}).get(run_id, "unknown")
        ...
        ax.plot(
            ...
            run["current_ma"][mask],
            linewidth=1.0,
            linestyle=status_linestyle(status),
            label=f"{run_id} ({status})",
        )

    if nominal_edge_s is not None:
        ax.axvline(
            nominal_edge_s,
            linestyle=":",
            linewidth=1.2,
    ...
    ylim = robust_ylim(runs, start_s, end_s)
    if ylim is not None:
        ax.set_ylim(*ylim)

    ax.set_title(f"{condition} | {title_suffix}")
    ax.set_xlabel("Time from active-window start [s]")
    ax.set_ylabel(runs[0]["ylabel"])
    ax.grid(True, alpha=0.25)
    ax.legend(ncol=2, fontsize=8)

    fig.tight_layout()

    out_path = out_dir / f"{condition}_{filename_suffix}.png"
...
def main() -> None:
    ...
    for condition, input_dir in INPUT_DIRS.items():
        ...
        runs = load_runs(condition, input_dir)

        if not runs:
            print("No runs found.")
            continue

        rise_times, fall_times = nominal_transition_times(condition)

        first_rise_s = float(rise_times[0])
        first_fall_s = float(fall_times[0])

        # 1) Full 20 s overlay
        plot_overlay(
            condition=condition,
            runs=runs,
            start_s=FULL_START_S,
            end_s=FULL_END_S,
            filename_suffix="full_active_overlay",
            title_suffix="all 10 runs | full active window",
        )

        # 2) First-burst rise zoom
        plot_overlay(
            condition=condition,
            runs=runs,
            start_s=max(0.0, first_rise_s - RISE_PRE_S),
            end_s=first_rise_s + RISE_POST_S,
            filename_suffix="first_burst_rise_zoom",
            title_suffix="all 10 runs | first-burst rise zoom",
            nominal_edge_s=first_rise_s,
        )

        # 3) First-burst fall zoom
        plot_overlay(
            condition=condition,
            runs=runs,
            start_s=max(0.0, first_fall_s - FALL_PRE_S),
            end_s=first_fall_s + FALL_POST_S,
            filename_suffix="first_burst_fall_zoom",
            title_suffix="all 10 runs | first-burst fall zoom",
            nominal_edge_s=first_fall_s,
        )
