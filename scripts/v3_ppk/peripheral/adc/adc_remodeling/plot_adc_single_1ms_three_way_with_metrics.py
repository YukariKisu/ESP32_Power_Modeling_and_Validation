...
# ============================================================
# Full active-window error metrics
# ============================================================

def build_full_active_predictions() -> tuple[
    np.ndarray,
    np.ndarray,
    np.ndarray,
]:
    """
    Simulate both input definitions continuously from t=0 to 20 s.

    Important:
    the ODE is initialized only once at the actual active-window start.
    It is NOT restarted inside a zoom/metric window.
    """
    time_s = np.arange(
        0.0,
        ACTIVE_DURATION_S,
    ...
    original_u = make_original_pulse_input(
        time_s
    )

    continuous_u = make_continuous_input(
        time_s
    )

    original_prediction = simulate_first_order_ode(
        time_s,
        original_u,
    )

    continuous_prediction = simulate_first_order_ode(
        time_s,
        continuous_u,
    )
...
def build_measured_active_mean(
    measured_runs,
    time_s: np.ndarray,
) -> np.ndarray:
    """
    Interpolate each measured run onto the same full active-window grid,
...
def calculate_error_metrics(
    ...
    measured = measured_ma[
        valid
    ]

    predicted = predicted_ma[
        valid
    ]
...
def calculate_full_active_metrics(
    measured_runs,
) -> pd.DataFrame:
    """
    Compare the original 44-us pulse input and the diagnostic continuous input
    over the SAME 20 s firmware-defined active window.
    """
    (
        time_s,
        original_prediction,
        continuous_prediction,
    ) = build_full_active_predictions()

    measured_mean = build_measured_active_mean(
        measured_runs,
        time_s,
    )

    original_metrics = calculate_error_metrics(
        measured_mean,
        original_prediction,
    )

    continuous_metrics = calculate_error_metrics(
        measured_mean,
        continuous_prediction,
    )
    ...
    rows = [
        {
            "case": "Original 44us pulse / 1ms",
            "input_definition": "44 us ON every 1 ms",
            "delta_i_ma": DELTA_I_MA,
            "tau_rise_ms": TAU_RISE_S * 1000.0,
            "tau_fall_ms": TAU_FALL_S * 1000.0,
            **original_metrics,
        },
        {
            "case": "Continuous input",
            "input_definition": "u=1 throughout active window",
            "delta_i_ma": DELTA_I_MA,
            "tau_rise_ms": TAU_RISE_S * 1000.0,
            "tau_fall_ms": TAU_FALL_S * 1000.0,
            **continuous_metrics,
...
def main() -> None:
    ...
    metrics = calculate_full_active_metrics(
        measured_runs
    )

    metrics_path = (
        OUTPUT_DIR
        / "single_1ms_original_vs_continuous_metrics.csv"
    )
    ...
    print(
        "\n=== Full 20 s active-window metrics ==="
    )
