...
# ============================================================
# Rise-edge timing error: measured rise - predicted rise
# ============================================================

RISE_TIMING_OUTPUT_DIR = Path(
    "results/v3_ppk/peripheral/adc/final_predictioned/"
    "rise_edge_timing_early_middle_late"
)

# Half-width of local search window around each predicted 50% crossing.
# Wide enough for the observed few-ms mismatch, but much smaller than 100 ms period.
RISE_SEARCH_HALF_WIDTH_S = 0.015

# Smoothing only for measured-edge detection.
RISE_EDGE_SMOOTH_S = 0.00010


def crossing_time_50pct(
    time_s: np.ndarray,
    ...
    """
    Return the first upward 50% crossing inside the requested time interval.
    Linear interpolation is used between samples.
    """
    threshold_ma = low_ma + 0.5 * (high_ma - low_ma)
    ...
    if len(t) < 3:
        raise ValueError("Not enough samples in rise search window")

    for i in range(len(t) - 1):
        if y[i] < threshold_ma <= y[i + 1]:
            y0 = y[i]
            y1 = y[i + 1]
            ...
            frac = (threshold_ma - y0) / (y1 - y0)
            return float(t[i] + frac * (t[i + 1] - t[i]))

    raise ValueError("No upward 50% crossing found")


def measured_rise_crossing_near_prediction(
    dataframe: pd.DataFrame,
    sync_start_s: float,
    ...
    """
    Detect measured 50% rise near one predicted rise.

    Time is expressed relative to firmware-defined active-window start.
    The measured waveform is NOT shifted to match the prediction.
    """
    active_t_s = (
        dataframe["time_s"].to_numpy(dtype=float)
        - sync_start_s
    ...
    dt_s = float(np.median(np.diff(active_t_s)))
    smooth_n = max(1, round(RISE_EDGE_SMOOTH_S / dt_s))
    smooth_ma = rolling_mean(current_ma, smooth_n)

    # Use local samples before/after the expected transition to estimate
    # measured low/high levels independently of the model amplitude.
    search_lo = predicted_crossing_s - RISE_SEARCH_HALF_WIDTH_S
    search_hi = predicted_crossing_s + RISE_SEARCH_HALF_WIDTH_S

    local_mask = (
        (active_t_s >= search_lo)
        & (active_t_s <= search_hi)
    )

    if not np.any(local_mask):
        raise ValueError("No measured samples near predicted rise")

    lt = active_t_s[local_mask]
    ly = smooth_ma[local_mask]

    # For 1000-sample bursts the pulse is long; for 100-sample bursts it is short.
    # Robust local percentiles work inside this narrow edge-centered window.
    low_ma = float(np.percentile(ly, 20))
    high_ma = float(np.percentile(ly, 95))

    if high_ma - low_ma < 2.0:
        raise ValueError(
            f"Measured local rise amplitude too small: {high_ma-low_ma:.3f} mA"
        )

    return crossing_time_50pct(
        time_s=active_t_s,
        signal_ma=smooth_ma,
        low_ma=low_ma,
        high_ma=high_ma,
        search_start_s=search_lo,
        search_end_s=search_hi,
    )


def predicted_rise_crossings(
    condition: str,
    model_name: str,
) -> list[tuple[int, float]]:
    """
    Calculate predicted 50% rise crossing time for every firmware burst.

    Returns:
        [(burst_index, predicted_50pct_crossing_s), ...]
    where burst_index starts at 1.
    """
    pred_t_s, pred_delta_ma = build_prediction_for_active_window(
        condition,
        model_name,
    )
    ...
    burst_starts = np.arange(
        0.0,
        ACTIVE_DURATION_S,
        interval_s,
    )

    # Exclude the final edge if there is not enough active-window data.
    for burst_idx, nominal_start_s in enumerate(burst_starts, start=1):
        # Search only within the burst-on interval. A first-order rise should
        # cross 50% well before the pulse ends for these two workloads.
        search_start_s = nominal_start_s
        search_end_s = min(
            nominal_start_s + duration_s,
            ACTIVE_DURATION_S - UNIFORM_DT_S,
        )

        try:
            cross_s = crossing_time_50pct(
                time_s=pred_t_s,
                signal_ma=pred_delta_ma,
                low_ma=0.0,
                high_ma=MODEL_PARAMETERS[model_name]["delta_i_ma"],
        ...
        rows.append((burst_idx, cross_s))

    return rows


def classify_comparison_window(time_s: float) -> str | None:
    for name, (start_s, end_s) in COMPARISON_WINDOWS_S.items():
        if start_s <= time_s <= end_s:
            return name
    return None


def analyze_rise_timing(
    diagnostics: pd.DataFrame,
) -> pd.DataFrame:
    """
    Per-edge timing table:
        Δt_rise = t_measured,50% - t_predicted,50%
    """
    rows: list[dict[str, object]] = []

    for condition in sorted(diagnostics["condition"].unique()):
        condition_rows = diagnostics[
            diagnostics["condition"] == condition
        ].copy()

        condition_rows["run_number"] = (
            condition_rows["run_id"]
            .str.extract(r"(\d+)")
            .astype(int)
        )
        condition_rows = condition_rows.sort_values("run_number")

        for model_name in MODELS_TO_PLOT:
            predicted_edges = predicted_rise_crossings(
                condition,
                model_name,
            )

            # Keep only Early/Middle/Late edges.
            predicted_edges = [
                (burst_idx, pred_s, classify_comparison_window(pred_s))
                for burst_idx, pred_s in predicted_edges
                if classify_comparison_window(pred_s) is not None
            ]
            ...
            for row in condition_rows.itertuples(index=False):
                ...
                for burst_idx, pred_s, window_name in predicted_edges:
                    try:
                        measured_s = measured_rise_crossing_near_prediction(
                            dataframe=dataframe,
                            sync_start_s=float(row.actual_sync_start_s),
                            predicted_crossing_s=pred_s,
                            condition=condition,
                        )
                        ...
                        rows.append(
                            {
                                ...
                                "window": window_name,
                                "predicted_rise_50_s": pred_s,
                                "measured_rise_50_s": measured_s,
                                "delta_t_rise_ms": delta_ms,
                            }
                        )
                        ...
                        rows.append(
                            {
                                ...
                                "window": window_name,
                                "predicted_rise_50_s": pred_s,
                                "measured_rise_50_s": np.nan,
                                "delta_t_rise_ms": np.nan,
                                "error": str(exc),
                            }
                        )
...
def summarize_rise_timing(
    edge_df: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    valid = edge_df[
        np.isfinite(edge_df["delta_t_rise_ms"])
    ].copy()

    per_run_window = (
        valid.groupby(
            ["condition", "model_name", "run_id", "window"],
            sort=True,
        )
        .agg(
            n_edges=("delta_t_rise_ms", "count"),
            delta_t_rise_mean_ms=("delta_t_rise_ms", "mean"),
            delta_t_rise_std_ms=("delta_t_rise_ms", "std"),
            delta_t_rise_min_ms=("delta_t_rise_ms", "min"),
            delta_t_rise_max_ms=("delta_t_rise_ms", "max"),
        )
        .reset_index()
    )
    ...
    per_run_window["window"] = per_run_window["window"].astype(window_order)
    per_run_window = per_run_window.sort_values(
        ["condition", "model_name", "run_id", "window"]
    )

    condition_window = (
        valid.groupby(
            ["condition", "model_name", "window"],
            sort=True,
        )
        .agg(
            n_edges=("delta_t_rise_ms", "count"),
            delta_t_rise_mean_ms=("delta_t_rise_ms", "mean"),
            delta_t_rise_std_ms=("delta_t_rise_ms", "std"),
            delta_t_rise_median_ms=("delta_t_rise_ms", "median"),
        )
        .reset_index()
    )
    condition_window["window"] = condition_window["window"].astype(window_order)
    condition_window = condition_window.sort_values(
        ["condition", "model_name", "window"]
    )

    return per_run_window, condition_window


def plot_rise_timing_early_middle_late(
    per_run_window: pd.DataFrame,
    condition: str,
    model_name: str,
) -> Path:
    subset = per_run_window[
        (per_run_window["condition"] == condition)
        & (per_run_window["model_name"] == model_name)
    ].copy()
    ...
    for run_id, group in subset.groupby("run_id"):
        ...
        y = group["delta_t_rise_mean_ms"].to_numpy(dtype=float)
    ...
    ax.set_ylabel(
        "Δt rise = measured 50% - predicted 50% [ms]"
    )
    ax.set_title(
        f"{condition} | {model_name} | rise-edge timing"
    )
    ax.grid(True, alpha=0.25)

    # 10 runs is still manageable, but place legend outside.
    ax.legend(
    ...
    out_path = (
        out_dir
        / f"{condition}_{model_name}_rise_timing_early_middle_late.png"
    )
...
def plot_rise_timing_vs_elapsed(
    edge_df: pd.DataFrame,
    condition: str,
    model_name: str,
) -> Path:
    subset = edge_df[
        (edge_df["condition"] == condition)
        & (edge_df["model_name"] == model_name)
        & np.isfinite(edge_df["delta_t_rise_ms"])
    ].copy()
    ...
    for run_id, group in subset.groupby("run_id"):
        group = group.sort_values("predicted_rise_50_s")

        ax.plot(
            group["predicted_rise_50_s"],
            group["delta_t_rise_ms"],
            marker=".",
            linewidth=0.9,
            alpha=0.75,
            label=run_id,
    ...
    ax.set_xlabel(
        "Elapsed time from firmware ADC active-window start [s]"
    )
    ax.set_ylabel(
        "Δt rise = measured 50% - predicted 50% [ms]"
    )
    ax.set_title(
        f"{condition} | {model_name} | rise timing vs elapsed time"
    )
    ax.grid(True, alpha=0.25)
    ax.legend(
        bbox_to_anchor=(1.02, 1.0),
    ...
    out_path = (
        out_dir
        / f"{condition}_{model_name}_rise_timing_vs_elapsed.png"
    )
...
def run_rise_timing_diagnostic(
    diagnostics: pd.DataFrame,
) -> None:
    RISE_TIMING_OUTPUT_DIR.mkdir(
        parents=True,
        exist_ok=True,
    )

    edge_df = analyze_rise_timing(
        diagnostics
    )

    edge_path = (
        RISE_TIMING_OUTPUT_DIR
        / "rise_edge_timing_per_edge.csv"
    )
    edge_df.to_csv(
        edge_path,
        index=False,
    ...
    per_run_window, condition_window = summarize_rise_timing(
        edge_df
    )

    per_run_path = (
        RISE_TIMING_OUTPUT_DIR
        / "rise_edge_timing_per_run_window.csv"
    )
    per_run_window.to_csv(
        per_run_path,
        index=False,
    )
    ...
    summary_path = (
        RISE_TIMING_OUTPUT_DIR
        / "rise_edge_timing_summary.csv"
    )
    condition_window.to_csv(
        summary_path,
        index=False,
    )

    print("\n=== Rise-edge timing summary ===")
    print(
        condition_window.to_string(
            index=False
        )
    )

    for condition in sorted(edge_df["condition"].unique()):
        for model_name in MODELS_TO_PLOT:
            p1 = plot_rise_timing_early_middle_late(
                per_run_window,
                condition,
                model_name,
            )
            p2 = plot_rise_timing_vs_elapsed(
                edge_df,
                condition,
                model_name,
            )
...
def main() -> None:
    ...
    # Quantify the visual Early/Middle/Late timing mismatch.
    run_rise_timing_diagnostic(
        diagnostics
    )
