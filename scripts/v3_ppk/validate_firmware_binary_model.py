...
# Firmware workload settings
FIRMWARE_CYCLE_US = 100000  # 100 ms cycle
...
# This correction makes firmware u(t) start slightly before aligned t=0.
...
def create_firmware_binary_input(time_grid, duty_percent):
    """
    Create firmware-defined binary input u(t).

    Firmware behavior:
        cycle_us = 100000
        busy_us = cycle_us * duty_percent / 100
        idle/wait_us = cycle_us - busy_us

    u(t) = 1 during busy section
    u(t) = 0 during wait section

    Since t = 0 is detected from current threshold crossing, not GPIO marker,
    optional phase correction shifts the firmware input start slightly earlier.
    """

    cycle_s = FIRMWARE_CYCLE_US / 1_000_000.0
...
def simulate_first_order_model_binary_input(time_grid, u_t):
    ...
    Exact discrete update is used for zero-order-hold input.
    """
...
def save_plot_for_intensity(
    ...
    """
    Save measured vs predicted plots.
    """
    ...
    plt.title(
        f"CPU {intensity}%: firmware-defined binary input ODE validation"
    )
    plt.xlim(-2, 12)
    plt.ylim(40, 90)
    plt.grid(True)
    ...
    out_plot = os.path.join(
        out_dir,
        f"ppk_cpu_{intensity}_firmware_binary_input_measured_vs_predicted.png",
    )
    ...
    plt.ylabel("Current [mA]")
    plt.title(
        f"CPU {intensity}%: switching zoom "
        "(firmware-defined binary input)"
    )
    plt.xlim(-0.02, 0.25)
    plt.ylim(40, 90)
    plt.grid(True)
    ...
    out_zoom_plot = os.path.join(
        out_dir,
        f"ppk_cpu_{intensity}_firmware_binary_input_zoom.png",
    )
...
for intensity in INTENSITIES:
    ...
    for csv_path in files:
        ...
        effective_firmware_duty = (
            np.mean(firmware_u_t[active_mask])
    ...
    out_run_summary = os.path.join(
        out_dir,
        f"ppk_cpu_{intensity}_firmware_binary_input_validation_by_run.csv",
    )
    ...
    out_mean = os.path.join(
        out_dir,
        f"ppk_cpu_{intensity}_firmware_binary_input_mean_waveform.csv",
    )
...
out_summary = os.path.join(
    BASE_OUT_DIR,
    "cpu_firmware_binary_input_validation_summary_cpu75_cpu50_cpu25.csv",
)

summary_df.to_csv(out_summary, index=False)

print("\n" + "=" * 70)
print("Combined firmware-defined binary-input validation summary")
print("=" * 70)
print(summary_df)
