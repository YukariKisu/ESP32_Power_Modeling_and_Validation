...
static esp_err_t read_register_separate(uint8_t reg, uint16_t *value)
{
    uint8_t data[2] = {0};
    ...
    // Step 2: read two bytes from that register
...
{
    uint16_t value = 0;
    esp_err_t ret = read_register_separate(reg, &value);
...
void app_main(void)
{
    printf("INA226 old API separate write/read test started\n");
    ...
    while (1) {
        printf("Writing config 0x%04X to register 0x00...\n", CONFIG_VALUE);

        esp_err_t write_ret = write_register(REG_CONFIG, CONFIG_VALUE);
        ...
        print_register(REG_POWER, "Power");
