...
/* 0 = idle phase, 1 = UART-enabled idle phase */
...
static volatile uint32_t g_idle_counter = 0;
...
/* ---------------- UART enabled-idle init ---------------- */

static void uart_enabled_idle_init(void)
{
...
/* ---------------- UART enabled-idle task on Core 1 ---------------- */

static void uart_enabled_idle_task(void *parameter)
{
    (void)parameter;

    while (g_experiment_running) {
        if (g_workload_state == 1) {
            /*
               UART is initialized and enabled, but no bytes are transmitted.
               This phase measures UART-enabled idle baseline current.
            */
            g_idle_counter++;
...
void app_main(void)
{
    ...
    printf("# experiment=ppk2_uart_enabled_idle_with_cpu_sync_pulse\n");
    printf("# measurement_device=PPK2\n");
    printf("# internal_current_sensor=disabled\n");
    printf("# purpose=UART initialized/enabled, no TX transmission\n");
    ...
    printf("# uart_transmission=disabled\n");
    ...
    /*
       UART is initialized before the measured idle/active/idle sequence.
       No uart_write_bytes() is called anywhere in this firmware.
    */
    uart_enabled_idle_init();
...
#if ENABLE_PHASE_LOGS
    printf("# phase=uart_enabled_idle\n");
#endif
    g_workload_state = 1;
    marker_set(1);
    ...
    printf("# uart_total_bytes_written=0\n");
    printf("# uart_enabled_idle_counter=%lu\n", (unsigned long)g_idle_counter);
